...
3. **CLI / human** — falls back to OS user + hostname + git config
   email + git HEAD sha if the cwd is a repo.
...
def _git_metadata() -> dict[str, Any]:
    ...
    if sha:
        out["git_sha"] = sha
        out["git_branch"] = _git("rev-parse", "--abbrev-ref", "HEAD")
        out["git_email"]  = _git("config", "user.email")
        out["git_dirty"]  = bool(_git("status", "--porcelain"))
    return out
